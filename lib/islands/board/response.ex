defmodule Islands.Board.Response do
  ...
  """
  @spec check_all_islands(Board.t(), Coord.t()) :: guess_check
  def check_all_islands(%Board{} = board, %Coord{} = guess) do
  ...
  """
  @spec full_response(guess_check, Board.t()) :: t
  def full_response({:hit, island} = _guess_check, %Board{} = board) do
    board = put_in(board.islands[island.type], island)
    {:hit, forest_check(island), win_check(board), board}
  end

  def full_response({:miss, guess} = _guess_check, %Board{} = board) do
